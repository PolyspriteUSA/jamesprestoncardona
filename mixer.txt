...
function buildAssignableMixer(){
  ...
  mixerChannelState.forEach((state,index)=>{
    ...
    const faderSlot=document.createElement("div");
    faderSlot.className="mixer-fader-slot";

    const faderScale=document.createElement("span");
    faderScale.className="mixer-fader-scale";
    ...
    faderSlot.append(faderScale,fader);
    faderWrap.append(faderLabel,faderSlot,faderOut);

    strip.append(channelHead,knobStack,faderWrap);
    mixer.appendChild(strip);
  });

  // Hard guard: the hardware mixer is exactly channels 1–8.
  Array.from(mixer.children).slice(MIXER_CHANNEL_COUNT).forEach(node=>node.remove());
  Array.from(mixer.querySelectorAll(".mixer-console-strip")).forEach((strip,index)=>{
    if(index>=MIXER_CHANNEL_COUNT){ strip.remove(); return; }
    strip.dataset.channelIndex=String(index);
    const number=strip.querySelector(".mixer-channel-head strong");
